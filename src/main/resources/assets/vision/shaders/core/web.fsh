...
void main() {
    ...
    vec2 uv = vec2(localUv.x, FlipV > 0.5 ? 1.0 - localUv.y : localUv.y);
    vec4 page = texture(Sampler0, uv);
    vec3 col = page.rgb;
    ...
    float outA = max(page.a, edge);
    fragColor = vec4(col, outA * shape * ColorModulator.a);
}
